                ...
                <div className='flex justify-between items-center text-base gap-8'>
                    ...
                    <ul className='md:flex space-x-12 hidden navitems'>
                        {navItems.map(({ link, path }) => (
                            <Link to={path} activeClass='active' spy={true} smooth={true} offset={-100} key={path} className='block text-base text-white uppercase hover:text-orange cursor-pointer'>{link}</Link>
                        ))}
                    </ul>
